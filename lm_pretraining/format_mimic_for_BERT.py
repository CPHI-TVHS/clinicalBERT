...
OUTPUT_DIR = r'U:/Workspaces/westerd/clinicalBERT/tokenized_notes'  # this path will contain tokenized notes. This dir will be the input dir for create_pretrain_data.sh
MIMIC_NOTES_FILE = r'N:\exports\noteevents'  # this is the path to mimic data if you're reading from a csv. Else uncomment the code to read from database below
...
sql_conn = pyodbc.connect('DRIVER={ODBC Driver 13 for SQL Server};SERVER=cph002wp-as.ea.vanderbilt.edu;DATABASE=MIMIC-III-v14-Full;Trusted_Connection=yes')
notes_query = "(select top 1000 * from mimiciii.noteevents);"
notes = pd.read_sql_query(notes_query, sql_conn)
